...
const reactionsApi = 'https://api.giphy.com/v1/gifs/search?api_key=nhTKuYXaMk6fp1e6S3Ts68zTe4nEmEN0&q=entertainment&limit=50' 
// <------------------------------------------------->

let entertainmentFetch = function(){
    fetch(reactionsApi).then((response)=>{
        if(response.ok){
            return response.json()
        }else{
            throw new Error('Network resposer error')
        }
    }).then((data)=>{
        console.log(data)
        displayData(data)
